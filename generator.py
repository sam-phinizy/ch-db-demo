...
class EcommerceDataGenerator:
    ...
    def _generate_hourly_sales(self, date: datetime, product: Dict) -> Dict:
        ...
        return {
            ...
            'total_revenue': round(base_quantity * product['base_price'] * price_multiplier, 2),
            ...
            for hour in range(24):
                ...
                for category, products in products_by_category.items():
                    for product in products:
                        ...
                        dir_path = Path(output_dir) / str(current_datetime.year) / \
                                 f"{current_datetime.month:02d}" / f"{current_datetime.day:02d}" / \
                                 f"{hour:02d}" / category.lower().replace(' & ', '_and_').replace(' ', '_')
                        
                        dir_path.mkdir(parents=True, exist_ok=True)
                        
                        # Generate hourly data
                        sales_data = self._generate_hourly_sales(current_datetime, product)
...
def main():
    ...
    parser.add_argument('--products', 
                       type=int, 
                       default=10,
                       help='Number of products (default: 10)')
    parser.add_argument('--months', 
                       type=int, 
                       default=1,
                       help='Number of months of data (default: 1)')
